...
void listarTabela(TlistaTabela *TB);
void inserirTabela(TlistaTabela *TB);
...
cadastrarAposta(TlistaCliente *L);
main(){
...
void inserirTabela(TlistaTabela *TB){ 
	system("cls");
    puts("+------------------------------------+");
    puts("|       02 - CADASTRAR TABELA        |");
    puts("+------------------------------------+");
     
    Ttabela *novo;

    novo = (Ttabela*) malloc(sizeof(Ttabela));
        
	fflush(stdin);
    printf("TIME(1): ");
	gets(novo->time1);
    printf("PLACAR(1): ");    
    scanf("%d", &novo->placar1);
    fflush(stdin);
    printf("TIME(2): ");
    gets(novo->time2);
    printf("PLACAR(2): ");
    fflush(stdin);
    scanf("%d", &novo->placar2);   
    novo->prox = NULL;
...
void listarTabela(TlistaTabela *TB){
    ...
    while(novo != NULL){ 
        printf("TIME1(%s)[%d] X TIME2(%s)[%d]:\n", novo->time1, novo->placar1, novo->time2, novo->placar2); 
...
void listarGanhador(TlistaCliente *L){
	system("cls");
    puts("+------------------------------------+");
    puts("|         04- O GANHADOR EH:         |");
    puts("+------------------------------------+");
    ...
    while(novo != NULL){ 
        cont_grana = cont_grana + novo->valor;     
        novo = novo->prox;
        puts("||||||||||||||||||||||||||||||||||||"); 
	} 
    printf("NOME GANHADOR:!!\n");
...
void salvarBL(TlistaCliente *L){
     ...
     arq = fopen("bolao.dat", "wb");
        fwrite(&L, sizeof(struct cliente), 1, arq);
        fclose(arq);     
}
...
void carregarBL(TlistaCliente *L){
     ...
     if(arq != NULL ) {
          fread(L, sizeof(struct cliente), 1, arq);
          fclose(arq);     
     }
}
